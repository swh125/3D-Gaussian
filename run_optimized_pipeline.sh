#!/usr/bin/env bash
# 一键运行脚本 - 优化版本（包含边缘优化）
# 使用方法：修改下面的路径，然后运行: bash run_optimized_pipeline.sh
#
# 优化内容：
#   - 2D mask渲染时自动应用形态学操作（去除边缘光晕和锯齿）
#   - 保持原视频顺序（所有处理过程都按文件名排序）
#   - 支持训练/测试集划分（最后N帧作为测试集）
...
echo ""
echo "优化内容:"
echo "  ✅ 2D mask渲染时自动应用形态学操作"
echo "  ✅ 开运算（kernel=2）：去除边缘光晕"
echo "  ✅ 闭运算（kernel=3）：平滑边缘，去除锯齿"
echo "  ✅ COLMAP自动质量检查和重试（使用最宽松参数：16384特征点）"
echo ""
...
echo ""
echo "优化说明:"
echo "  - 2D mask渲染已自动应用形态学优化"
echo "  - 边缘光晕和锯齿已减少"
echo "  - 渲染mask时使用: python render.py -m ${MODEL_PATH} -s ${OUTPUT_DIR} --target seg"
echo ""
